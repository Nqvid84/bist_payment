...
const networksSummeryComponent = () => {
  ...
  const fetchSummary = async () => {
    ...
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/microservice/v1/payment/server/network/transactions/report${queryParams ? `?${queryParams}` : ''}`,
  ...
  const handleClear = () => {
    setSearchParamsState({ dateFrom: '', dateTo: '' });
    navigate('/payment-card/network-transactions-report');
  };
              ...
              <CardBody className="relative p-6">
                ...
                <div className="mt-6 flex justify-between border-t border-slate-700/50 pt-4">
                  <Tooltip delay={1000} content="کپی نام شبکه">
                    <Button variant="light" size="sm" onClick={() => copyToClipboard(item.name)} className="text-primary-400 hover:text-primary-300" endContent={<FiCopy size={14} />}>
                      نام
                    </Button>
                  </Tooltip>
                  <Tooltip delay={1000} content="کپی نام دستگاه">
                    <Button variant="light" size="sm" onClick={() => copyToClipboard(item.deviceName)} className="text-primary-400 hover:text-primary-300" endContent={<FiCopy size={14} />}>
                      دستگاه
                    </Button>
                  </Tooltip>
                </div>
...
export const NetworksSummery = memo(networksSummeryComponent);